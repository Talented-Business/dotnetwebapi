...
namespace DataAccessLayer.Repositories
{
    public class CompanyRepository : ICompanyRepository
    {
        ...
        public async Task <IEnumerable<Company>> GetAsyncAll()
        {
            return await _companyDbWrapper.FindAllAsync();
        }

        public async Task<Company> GetAsyncByCode(string companyCode)
        {
            await Task.Delay(20);
            return _companyDbWrapper.Find(t => t.CompanyCode.Equals(companyCode))?.FirstOrDefault();
        }

        public async Task<bool> SaveAsyncCompany(Company company)
        {
            var itemRepo = _companyDbWrapper.Find(t =>
                t.SiteId.Equals(company.SiteId) && t.CompanyCode.Equals(company.CompanyCode))?.FirstOrDefault();
            if (itemRepo != null)
        ...
        public async Task<bool> DeleteAsyncCompany(string companyCode) {
